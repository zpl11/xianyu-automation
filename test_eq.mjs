...
(async () => {
  ...
  const r1 = await callMTOP('idle.web.xyh.item.list', { needGroupInfo: true, pageNumber: 1, userId: '2222621138424', pageSize: 20 }, { spm_cnt: 'a21ybx.personal.0.0' });
  const ids1 = (r1.data?.cardList || []).map(c => c.cardData?.detailParams?.itemId).join(',');
  console.log('Page 1 IDs:', ids1);
  
  const r2 = await callMTOP('idle.web.xyh.item.list', { needGroupInfo: true, pageNumber: 2, userId: '2222621138424', pageSize: 20 }, { spm_cnt: 'a21ybx.personal.0.0' });
  const ids2 = (r2.data?.cardList || []).map(c => c.cardData?.detailParams?.itemId).join(',');
  console.log('Page 2 IDs:', ids2);
  
  console.log('Same items?', ids1 === ids2);
})();

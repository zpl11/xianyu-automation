...
(async () => {
  ...
  for (let page = 1; page <= 15; page++) {
    const r = await callMTOP('idle.web.xyh.item.list', { needGroupInfo: true, pageNumber: page, userId: '2222621138424', pageSize: 20 }, { spm_cnt: 'a21ybx.personal.0.0' });
    const cards = r.data?.cardList || [];
    let newCount = 0;
    cards.forEach(c => {
      const id = c.cardData?.detailParams?.itemId;
      if (id && !seenIds.has(id)) {
        seenIds.add(id);
        newCount++;
      }
    });
    console.log('Page', page, 'new items:', newCount, 'total unique:', seenIds.size);
    if (newCount === 0) {
      console.log('No new items, breaking at page', page);
      break;
    }
  }
})();

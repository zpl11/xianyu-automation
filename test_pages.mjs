...
(async () => {
  ...
  let r = await callMTOP('idle.web.xyh.item.list', { needGroupInfo: true, pageNumber: 1, userId: '2222621138424', pageSize: 20 }, { spm_cnt: 'a21ybx.personal.0.0' });
  console.log('Page 1 count:', r.data?.cardList?.length, 'nextPage:', r.data?.nextPage);
  r = await callMTOP('idle.web.xyh.item.list', { needGroupInfo: true, pageNumber: 2, userId: '2222621138424', pageSize: 20 }, { spm_cnt: 'a21ybx.personal.0.0' });
  console.log('Page 2 count:', r.data?.cardList?.length, 'nextPage:', r.data?.nextPage);
})();
